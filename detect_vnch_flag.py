...
model_path = r'runs\train\exp10\weights\best.pt'  # Thay đổi theo đường dẫn của bạn
# Tải model
model = torch.hub.load('ultralytics/yolov5', 'custom', path=model_path)

st.title("DETECT VNCH FLAG")
...
if uploaded_file is not None:
    ...
    if uploaded_file.type in ['image/jpeg', 'image/png']:
        ...
        if any(class_names[int(cls)] == 'vnch_flag' for cls in detected_classes):
            detected = True

        # Hiển thị kết quả
        st.image(results.render()[0], caption='Detect', use_column_width=True)
        if detected:
            st.write("**Discovered VNCH Flag in this image!**")
        else:
            st.write("**Nothing**")
        ...
        if detected:
            st.write("**Discovered VNCH Flag in this video!**")
